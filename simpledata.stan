...
parameters {
  ...
  real beta;  // coef of eco state in p state eqn
...
model {
  ...
  for(t in 3:T) {
    Y[t,1] ~ normal(xhat[t]*gamma[1] + phat[t], 1); //nominal monthly variable

    Y[t,2] ~ normal(xhat[t]*gamma[2], 1); //real monthly variable

    if(Y[t,3] != -999) { 
      Y[t,3] ~ normal( (3^-1)*sum(xhat[(t-2):t])*gamma[3] + (3^-1)*sum(phat[(t-2):t]), 1); //qtly nom variable
    }
    
    if(Y[t,4] != -999) { 
      Y[t,4] ~ normal( (3^-1)*sum(xhat[(t-2):t])*gamma[4], 1); //qtly real variable
    }
    
    if(Y[t,5] != -999) { 
      Y[t,5] ~ normal( (3^-1)*sum(phat[(t-2):t])*lambda, 1); //qtly price variable
    }
  }
}
